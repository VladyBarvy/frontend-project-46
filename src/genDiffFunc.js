  ...
  const buildMyTree = (file1, file2) => {
    ...
    const differences = allKeys.map((key) => {
      ...
      if (_.isObject(file1[key]) && _.isObject(file2[key])) {
        return { key, type: 'nested', value: buildMyTree(file1[key], file2[key]) };
      }



      if (!_.isEqual(file1[key], file2[key])) {
        return { key, value1: file1[key], value2: file2[key], type: 'updated' };
      }
...
*/
 ...
 //return formatter(buildMyTree(firstFile, secondFile), formatType);
};
